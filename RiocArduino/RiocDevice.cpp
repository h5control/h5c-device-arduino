...
void setupRioc(byte msg[8], byte address_from);
...
void setupRioc(byte msg[8], byte address_from) 
{
  ...
  if (type==RO_GENERAL_DIGITAL_IN) {
    ...
    if (pinAvailable(msg[2]) && pinAvailable(msg[3]) && pinAvailable(msg[4]) && pinAvailable(msg[5])) obj = (RiocObject*)(new ROStepper());

  } else if (type==RO_MOTION_SERVO) {

    if (pinAvailable(msg[2])) obj = (RiocObject*)(new ROServo());    
